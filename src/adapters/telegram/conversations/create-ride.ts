...
export function buildCreateRideConversation(rideService: RideService) {
  return async (conversation: Conv, ctx: Ctx): Promise<void> => {
    ...
    const proposerName = from.last_name != null ? `${from.first_name} ${from.last_name}` : from.first_name
    ...
    const notes = prefill.notes ?? await askOptionalText(conversation, ctx, "📝 Notes? (/skip to leave blank)")

    const confirmed = await stepConfirm(conversation, ctx, {
      date, meetingTime, meetingPoint, name: name ?? undefined,
      proposerName, ...stats, notes: notes ?? undefined,
    ...
    await rideService.propose({
      ...
      name: name ?? undefined, date, meetingTime, meetingPoint,
      distanceKm: stats.distanceKm, elevationGain: stats.elevationGain,
      elevationLoss: stats.elevationLoss, level,
      externalUrl: prefill.externalUrl, notes: notes ?? undefined,
    })
...
async function stepImport(conversation: Conv, ctx: Ctx): Promise<Prefill> {
  ...
  try {
    ...
    const warning =
      err instanceof UnsupportedPlatformError ? "⚠️ Platform not supported — continuing manually."
      : err instanceof ExtractionFailedError ? "⚠️ Could not extract details — continuing manually."
      : ""
...
async function stepDate(
  ...
  while (true) {
    const msg = await conversation.waitFor("message:text", { maxMilliseconds: TIMEOUT_MS })
    if (msg == null) { await ctx.reply("⏱ Ride creation timed out."); return null }
    const parsed = parseDateAndTime(msg.message.text)
    if (parsed != null) return parsed
    await ctx.reply("❌ Invalid format. Use <code>DD/MM/YYYY</code> or <code>DD/MM/YYYY HH:MM</code>.", {
      parse_mode: "HTML",
    })
  }
}

async function stepStats(conversation: Conv, ctx: Ctx, prefill: Prefill): Promise<{
  distanceKm?: number; elevationGain?: number; elevationLoss?: number; level?: RideLevel
}> {
  if (prefill.distanceKm != null || prefill.elevationGain != null) {
    return { distanceKm: prefill.distanceKm, elevationGain: prefill.elevationGain, elevationLoss: prefill.elevationLoss, level: prefill.level }
  }
  await ctx.reply(
...
async function stepConfirm(
  ...
  const kb = new InlineKeyboard().text("✅ Create ride", "confirm:yes").text("❌ Cancel", "confirm:no")
  ...
  if (cbq == null) { await ctx.reply("⏱ Ride creation timed out."); return false }
  await cbq.answerCallbackQuery()
  if (cbq.callbackQuery.data === "confirm:no") { await ctx.reply("Ride creation cancelled."); return false }
  return true
}
...
async function askOptionalText(conversation: Conv, ctx: Ctx, prompt: string): Promise<string | null> {
  await ctx.reply(prompt)
  const msg = await conversation.waitFor("message:text", { maxMilliseconds: TIMEOUT_MS })
