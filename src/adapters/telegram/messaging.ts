...
export class TelegramMessaging implements MessagingPort {
  ...
  async pinSummary(threadId: ThreadId, ride: Ride): Promise<number> {
    ...
    await this.api.pinChatMessage(this.groupChatId, msg.message_id, { disable_notification: true })
    return msg.message_id
  }

  async updatePinnedSummary(_threadId: ThreadId, ride: Ride): Promise<void> {
    if (!ride.pinnedMessageId) return
    await this.api.editMessageText(this.groupChatId, ride.pinnedMessageId, formatSummary(ride), {
      parse_mode: "HTML",
    })
  }
  ...
  async removeMemberFromThread(_threadId: ThreadId, _userId: UserId): Promise<void> {
    // No-op: topic visibility equals group membership on Telegram.
  }
