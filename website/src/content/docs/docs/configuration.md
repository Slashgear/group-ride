...
1. The **start coordinates of an imported GPX track**, if the ride was created from a `.gpx` file or link — or from a `.gpx` posted afterward in the ride's Discord thread (see [Commands → GPX upload after creation](/docs/commands/#gpx-upload-after-creation-discord)) — most accurate, set automatically.
